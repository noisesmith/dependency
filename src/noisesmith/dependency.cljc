...
(deftype MapDependencyGraph [dependencies dependents]
  ...
  (depend [graph node dep]
    (when (or (= node dep) (depends? graph dep node))
      (throw #?(:clj (Exception. (str "Circular dependency between "
                                      (pr-str node) " and " (pr-str dep)))
                :cljs (js/Error. (str "Circular dependency between "
                                      (pr-str node) " and " (pr-str dep))))))
    (MapDependencyGraph.
...
(defn topo-comparator
  ...
  (let [pos (zipmap (topo-sort graph) (range))]
    (fn [a b]
      (compare (get pos a #?(:clj Long/MAX_VALUE :cljs Infinity))
               (get pos b #?(:clj Long/MAX_VALUE :cljs Infinity))))))
